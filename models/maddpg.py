...
class MADDPG:
    ...
    def take_action(self, states, explore):
        states = [torch.tensor([states[i]], dtype=torch.float, device=self.device) for i in range(len(self.env.agents))]
    ...
    def update(self, sample, i_agent):
        ...
        # hoho: 如何体现出中心化小樽了critic网络？使用所有agent的动作作为critic的输入来预测价值
        ...
        cur_actor_out = cur_agent.actor(obs[i_agent])  # hoho: 如何体现只训练自身actor网络？只使用当前agent对状态的部分观测预测动作！
        ...
        actor_loss += (cur_actor_out**2).mean() * 1e-3   # hoho_todo: 这里策略梯度更新好奇怪！平方是啥意思？
        actor_loss.backward()
...
if __name__ == '__main__':
    # logtis = np.array([[1, 2, 3], [4, 5, 6]])
    # print(torch.eye(logtis.shape[1])[np.random.choice(range(logtis.shape[1]), size=logtis.shape[0])])
    # print(torch.rand(logtis.shape[0]))
    # print(logtis == logtis.max(1, keepdims=True)[0])

    num_episodes = 5000
    episode_length = 25  # 每条序列的最大长度（hoho: 时间步数）
    buffer_size = 100000
    hidden_dim = 64
    actor_lr = 1e-2
    critic_lr = 1e-2
